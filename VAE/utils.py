...
def preprocess_pair(img_a, img_b, fine_size=(256, 256), flip=True, is_test=False):
    load_size = np.zeros_like(fine_size)
    load_size[0] = fine_size[0] + int(np.ceil(0.1 * fine_size[0]))
    load_size[1] = fine_size[1] + int(np.ceil(0.1 * fine_size[1]))

    if is_test:
        img_a = scipy.misc.imresize(img_a, [fine_size[0], fine_size[1]])
        img_b = scipy.misc.imresize(img_b, [fine_size[0], fine_size[1]])
    else:
        img_a = scipy.misc.imresize(img_a, [load_size[0], load_size[1]])
        img_b = scipy.misc.imresize(img_b, [load_size[0], load_size[1]])
...
def load_data(image_path, flip=True, is_test=False, which_direction=0, is_gray_scale=True, img_size=(256, 256, 1)):
    ...
    img_a, img_b = preprocess_pair(img_a, img_b, fine_size=img_size, flip=flip, is_test=is_test)
    # VAE input should be [0, 1]
    img_a = img_a / 255.
    img_b = img_b / 255.
